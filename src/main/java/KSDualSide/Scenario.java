...
public class Scenario {
  ...
  void setKnowledgeOf(int focal) {
    BitSet diff = (BitSet) belief[focal].clone();
    diff.xor(reality);
    if( Main.S == 1 ){
      knowledge[focal] = Main.M - diff.cardinality();
    }else{
      knowledge[focal] = 0;
  ...
  void initializeRank() {
    ...
    for (int i = 0; i < Main.N; i++) {
      for (int j = i + 1; j < Main.N; j++) {
        if (knowledge[i] <= knowledge[j]) rank0[i]++;
        else rank0[j]++;
      }
    }
    for (int focal : focalIndexArray) {
  ...
  void setConcentration(){
    concentration = 0;
    int[] sumSourceCount = new int[Main.N];
    for( int focal : focalIndexArray ){
      for( int target : targetIndexArray ) {
        sumSourceCount[focal] += beliefSourceCount[target][focal];
      }
    }
    for( int focal : focalIndexArray ){
      double proportion = sumSourceCount[focal] / (double) Main.M_N;
