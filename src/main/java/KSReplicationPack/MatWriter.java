...
class MatWriter {
  ...
  MatWriter(Computation c) {
    Matrix innovativeCapabilityAVG = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix innovativeCapabilitySTD = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    ...
    Matrix optimalBetaSTD = Mat5.newMatrix(Main.RESULT_KEY_VALUE_OPTIMAL);
      ...
      for (int ps = 0; ps < Main.LENGTH_P_SHARING; ps++) {
        for (int t = 0; t < Main.TIME; t++) {
          for (int b = 0; b < Main.LENGTH_BETA; b++) {
            ...
            innovativeCapabilityAVG.setDouble(indices, c.innovativeCapabilityAVG[nt][ps][b][t]);
            innovativeCapabilitySTD.setDouble(indices, c.innovativeCapabilitySTD[nt][ps][b][t]);
            beliefDiversityAVG.setDouble(indices, c.beliefDiversityAVG[nt][ps][b][t]);
            beliefDiversitySTD.setDouble(indices, c.beliefDiversitySTD[nt][ps][b][t]);
            beliefSourceDiversityAVG.setDouble(indices, c.beliefSourceDiversityAVG[nt][ps][b][t]);
            beliefSourceDiversitySTD.setDouble(indices, c.beliefSourceDiversitySTD[nt][ps][b][t]);
            centralizationAVG.setDouble(indices, c.centralizationAVG[nt][ps][b][t]);
            centralizationSTD.setDouble(indices, c.centralizationSTD[nt][ps][b][t]);
            efficiencyAVG.setDouble(indices, c.efficiencyAVG[nt][ps][b][t]);
            efficiencySTD.setDouble(indices, c.efficiencySTD[nt][ps][b][t]);
            for (int n = 0; n < Main.N; n++) {
              ...
              rankContributionSTD.setDouble(indicesRank, c.rankContributionSTD[nt][ps][b][t][n]);
              rankContributionPositiveAVG.setDouble(indicesRank, c.rankContributionPositiveAVG[nt][ps][b][t][n]);
              rankContributionPositiveSTD.setDouble(indicesRank, c.rankContributionPositiveSTD[nt][ps][b][t][n]);
              rankContributionNegativeAVG.setDouble(indicesRank, c.rankContributionNegativeAVG[nt][ps][b][t][n]);
              rankContributionNegativeSTD.setDouble(indicesRank, c.rankContributionNegativeSTD[nt][ps][b][t][n]);
            }
          }
          int[] indices = {nt, ps, t};
          optimalBetaAVG.setDouble(indices, c.optimalBetaAVG[nt][ps][t]);
          optimalBetaSTD.setDouble(indices, c.optimalBetaSTD[nt][ps][t]);
        }
      }
    }
    ...
    try {
      MatFile matFile = mat5File
          ...
          .addArray("para_n_in", Mat5.newScalar(Main.N_FACTOR))
          .addArray("para_n_of", Mat5.newScalar(Main.N_PRIME))
          ...
          .addArray("r_inca_avg", innovativeCapabilityAVG)
          .addArray("r_inca_std", innovativeCapabilitySTD)
          ...
          .addArray("r_opti_std", optimalBetaSTD)
