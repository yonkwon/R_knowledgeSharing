...
## Figure X1. t -> Outcome @ beta; Seeker v. Sharer

```{r fig}
set_fig_network_type <- 1 # 1: Random spanning tree
set_fig_beta <- data$para.l.b
set_fig_p.s <- c(1, data$para.l.p.s)
set_fig_time_limit <- data$para.time
set_fig_time_limit <- 300
set_fig_time <- 1:set_fig_time_limit
set_fig_outcome <- data$r.know.avg[set_fig_network_type, set_fig_beta, set_fig_p.s, set_fig_time]
set_fig_outcome_label <- 'Firm Performance'

# set_fig_outcome <- data$r.bfdv.avg[set_fig_network_type, set_fig_beta, set_fig_p.s, set_fig_time]
# set_fig_outcome_label <- 'Belief Diversity'

set_fig_p.s_label <- c('Seeker Organization', 'Sharer Organization')
set_fig_p.s_label <- factor(set_fig2_p.s_label, levels = set_fig2_p.s_label)

df_fig <- data.frame()
for( p.s_idx in 1:2 ){
  df_next <- data.frame(
    p.s = paste0('p[sharing]==',data$para.a.p.s[p.s_idx]),
    p.s_label = set_fig_p.s_label[p.s_idx],
    time= set_fig_time,
    outcome = set_fig_outcome[p.s_idx,]
  )
  df_fig <- rbind(df_fig, df_next)
}

fig <- ggplot(df_fig, aes(x = time, y = outcome, color = p.s_label)) +
  geom_line() +
...
fig_params_label <- paste0(
  'fig',
  '(', label_network_type[set_fig_network_type],')',
  'b', data$para.a.b[set_fig_beta],
  'p.s', paste0(set_fig_p.s, collapse = ","),
  'res_', set_fig_outcome_label
)

ggsave(
  paste0(fig_id, '_', fig_params_label, '.png'),
  plot = fig,
  units = 'in', 
  width = 4,
  height = 2,
...
## Figure 1. beta to efficiency, multiple network types
...
```{r fig2}
set_fig2_network_type <- 1 # 1: Random spanning tree
set_fig2_beta <- data$para.l.b
set_fig2_p.s <- 1:data$para.l.p.s
# set_fig2_p.s <- c(1, 11, 21)
set_fig2_time_limit <- data$para.time
set_fig2_time_limit <- 300
...
files2print <- function(list_file) {
  for (file in list_file) {
    ...
    net_node$isSharer <<- ifelse(net_node$pSharing == 1, type_label[1], type_label[2])
